...
import { AnalyzeImageRequest } from "../const/request.schema";
import { GeminiServiceConfig } from "../const/gemini.config";
...
export class GeminiService {
  ...
  async analyzeImage(request: AnalyzeImageRequest): Promise<string> {
    try {
      ...
      if (error?.status === 400) {
        throw new Error(
          `Bad request: ${error.message || "Dữ liệu đầu vào không hợp lệ"}`
        );
      }

      if (error?.message?.includes("not found for API version")) {
        throw new Error(
          "Model không tồn tại ở API v1beta. Thử đổi sang gemini-1.5-flash-latest hoặc đặt biến môi trường GEMINI_MODEL."
        );
      }
      ...
      if (error?.message) {
        ...
        if (
          ...
          throw new Error(
            `Gemini API: ${error.message}. Kiểm tra quota tại: https://aistudio.google.com/app/apikey`
          );
        }
        if (
          ...
          throw new Error(
            `Gemini API: ${error.message}. Kiểm tra API key tại: https://aistudio.google.com/app/apikey`
          );
        }
